    ...
    GameState( hWnd, pRenderer ), m_MIDI( sMIDIFile ), m_eGameMode( eGameMode )
{
    ...
    for (auto note_state : m_vStateReversed)
        note_state.reserve(m_MIDI.GetInfo().iNumTracks * 16);
...
GameState::GameError MainScreen::Logic( void )
{
    ...
    if ( !m_bPaused )
    {
        ...
        if (m_llPrevTime > m_llStartTime && !Config::GetConfig().GetPlaybackSettings().GetPaused()) { // Handle time jump and negative playback speed from cheat engine
            JumpTo(m_llStartTime - (1e+6 / (m_dFPS / 4)), true);
        }
        else {
            if (abs(m_llStartTime - m_llPrevTime) > (1e+6 / (m_dFPS / 2))) {
                JumpTo(m_llStartTime - (1e+6 / (m_dFPS / 4)), true);
            }
            else {
                m_llPrevTime = m_llStartTime;
    ...
    // Song's over
    if (!m_bPaused && (m_llStartTime >= llMaxTime || m_llStartTime < llMinTime)) {
        if (m_bDumpFrames) {
            char buf[1<<10] = {};
...
{
    const auto DeadLine = chrono::high_resolution_clock::now() + chrono::milliseconds(1<<2);

    // Start position and current state
    auto itBegin = m_vEvents.begin();
    // Want lower bound to minimize simultaneous complexity
    auto itMiddle = m_vEvents.begin() + iPos;
            ...
            if (m_bDumpFrames || chrono::high_resolution_clock::now() < DeadLine) {
                ...
                if (pEvent->GetChannelEventType() == MIDIChannelEvent::NoteOn && pEvent->GetParam2() > 0 && pEvent->HasSister()) {
                    ...
                    if (pSister->GetAbsMicroSec() > (*itPrev)->GetAbsMicroSec()) // > because itMiddle is the max for its time
                        iFound++;
                    if (pSister->GetAbsMicroSec() > m_vEvents[iPos]->GetAbsMicroSec()) { // > because we don't care about simultaneous ending notes
                        (m_vStateReversed[pEvent->GetParam1()]).push_back(idx);
                    }
                }
            }
            else {
...
void MainScreen::JumpTo(long long llStartTime, boolean loadingMode)
{
    ...
    if (itMiddle != itBegin)
    {
        ...
        auto itPrev = itMiddle - 1;
        for (; itPrev != itBegin; itPrev--) {
            if (m_bDumpFrames || !loadingMode || chrono::high_resolution_clock::now() < DeadLine) {
                if ((*itPrev)->GetChannelEventType() == MIDIChannelEvent::NoteOn && (*itPrev)->GetParam2() > 0)
                    break;
            }
            else {
...
void MainScreen::RenderNotes()
{
    ...
    if (Config::GetConfig().GetPlaybackSettings().GetNSpeed() < 0) {
        for (int i = 0; i < 128; i++) {
            for (vector< int >::iterator it = (m_vStateReversed[i]).begin(); it != (m_vStateReversed[i]).end(); it++) {
                RenderNote(m_vEvents[*it]);
            }
        }
    }
...
void MainScreen::RenderStatus(LPRECT prcStatus)
{
    ...
    if (FrameCount % (1<<4) == 0) {
        ...
        cout << string(csbi.dwSize.X, ' ');
        SetConsoleCursorPosition(hConsole, pos);
        cout << "    SameWidthNotes: " << cVideo.bSameWidth << " (Byte +" + GetAddress(cVideo.bSameWidth) + ") [Read / Write]";
        pos.X = 0;
        pos.Y = line; line++;
        SetConsoleCursorPosition(hConsole, pos);
        cout << string(csbi.dwSize.X, ' ');
        ...
        cout << string(csbi.dwSize.X, ' ');
        SetConsoleCursorPosition(hConsole, pos);
        cout << "    VisualizePitchBends: " << cVideo.bVisualizePitchBends << " (Byte +" + GetAddress(cVideo.bVisualizePitchBends) + ")[Read / Write]";
        pos.X = 0;
        pos.Y = line; line++;
        SetConsoleCursorPosition(hConsole, pos);
        cout << string(csbi.dwSize.X, ' ');
        SetConsoleCursorPosition(hConsole, pos);
        cout << "    PhigrosMode: " << cControls.bPhigros << " (Byte +" + GetAddress(cControls.bPhigros) + ") [Read / Write]";
        pos.X = 0;
        pos.Y = line; line++;
        SetConsoleCursorPosition(hConsole, pos);
        cout << string(csbi.dwSize.X, ' ');
        SetConsoleCursorPosition(hConsole, pos);
        cout << "    ShowMarkers: " << cVideo.bShowMarkers << " (Byte +" + GetAddress(cVideo.bShowMarkers) + ") [Read / Write]";
        pos.X = 0;
        pos.Y = line; line++;
        SetConsoleCursorPosition(hConsole, pos);
        cout << string(csbi.dwSize.X, ' ');
        SetConsoleCursorPosition(hConsole, pos);
        cout << "    TickBased: " << cVideo.bTickBased << " (Byte +" + GetAddress(cVideo.bTickBased) + ") [Read / Write]";
        pos.X = 0;
        pos.Y = line; line++;
        SetConsoleCursorPosition(hConsole, pos);
        cout << string(csbi.dwSize.X, ' ');
        SetConsoleCursorPosition(hConsole, pos);
        cout << "    HideStatistics: " << cVideo.bDisableUI << " (Byte +" + GetAddress(cVideo.bDisableUI) + ") [Read / Write]";
        pos.X = 0;
        pos.Y = line; line++;
        SetConsoleCursorPosition(hConsole, pos);
        cout << string(csbi.dwSize.X, ' ');
